...
class _RepositoryHandler:
    def __init__(self, link: str):
        ...
        self.sufficiency_for_considering_contributors = args["sufficiency_for_considering_contributors"]
        self.sufficiency_for_considering_files = args["sufficiency_for_considering_files"]
    ...
    def _get_groups_of_developers_files(self) -> List[tuple]:
        top_authors_by_files = {file: list(map(lambda n_author: n_author[1], list(
            filter(lambda n_author: n_author[0] >= self.sufficiency_for_considering_contributors * max(
    ...
    def _get_groups_of_files(self) -> List[tuple]:
        ...
        a =  list(filter(lambda subset: res[subset] >= self.sufficiency_for_considering_files * max(res.values()), res.keys()))
        return a
